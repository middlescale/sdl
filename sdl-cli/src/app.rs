...
impl ServiceManager {
    fn apply_device_name_update(
        self: &Arc<Self>,
        applied_name: String,
    ) -> anyhow::Result<DeviceNameUpdate> {
    ...
    fn rename_device(self: &Arc<Self>, new_name: &str) -> anyhow::Result<String> {
        ...
        match runtime.request_device_rename(trimmed.to_string(), Duration::from_secs(10))? {
            RenameRequestOutcome::Applied(applied_name) => {
                let update = self.apply_device_name_update(applied_name.clone())?;
                if let Some(runtime_name) = update.runtime_name {
                    Ok(format!(
                        "device renamed to {} (runtime still {}, restart required)",
    ...
    fn info(&self) -> io::Result<Info> {
        match self.0.current_runtime() {
            Ok(vnt) => {
                ...
                let runtime_name = info.name.clone();
                if runtime_name != desired_name {
                    info.runtime_name = Some(runtime_name);
...
impl ServiceCallback {
    ...
    fn request_runtime_name_refresh(&self, new_name: String) {
        if let Some(manager) = self.manager.upgrade() {
            std::thread::spawn(
                move || match manager.apply_device_name_update(new_name.clone()) {
                    Ok(update) => {
                        if let Some(runtime_name) = update.runtime_name {
                            println!(
                                "{}",
